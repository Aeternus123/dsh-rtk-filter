...
export function flattenPlainText(content: ContentBlock[]): string | undefined {
  let text = ''
...
export function resolveRtkPath(
  command: string,
  candidateDirs: readonly string[] = DEFAULT_RTK_CANDIDATE_DIRS,
): string {
  if (command.includes('/')) return command
  if (resolvedPathCache?.command === command) return resolvedPathCache.path
...
export function runRtk(
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      child = spawn(command, [...args], { stdio: ['pipe', 'pipe', 'pipe'] as const, detached: true })
    } catch (error: unknown) {
    ...
    const settle = (result: RtkRunResult): void => {
      if (settled) return
      settled = true
      clearTimeout(timer)
    ...
    child.on('error', (error: Error) => settle({ text: undefined, failure: error.message }))
 ...
 */
export function buildCondensedBody(
  condensed: string,
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.on('tools/post-execute', async (exec: ToolExecution, result, next): Promise<PostToolDecision> => {
    ...
    if (!toolSet.has(exec.name)) return decision

    const content = decision.content ?? result.content
    const text = flattenPlainText(content)
    if (text === undefined) return decision
    const inputBytes = Buffer.byteLength(text, 'utf8')
    if (inputBytes < minBytes) return decision
    ...
    if (body.length === 0) return decision
    ...
    if (outcome.text === body) return decision
